...
- **`2026/05/26`**: 🎨 Интерфейс Gradio теперь поддерживает генерацию, редактирование и понимание изображений и видео. [Попробуйте!](assets/docs/changelog/2026-05-26.md)
- **`2026/05/25`**: ✨ [Hugging Face Space](https://huggingface.co/spaces/bytedance-research/Lance) запущен благодаря команде HF!
- **`2026/05/19`**: 🤗 Технический отчёт доступен на [arXiv](http://arxiv.org/abs/2605.18678).
- **`2026/05/18`**: 🔥 Запущена [домашняя страница проекта](https://lance-project.github.io/), опубликованы код вывода и веса модели на [GitHub](https://github.com/bytedance/Lance/) и [Hugging Face](https://huggingface.co/bytedance-research/Lance).
...
| `setup_env_torch28cuda126.bat` | **Альтернативная настройка окружения.** Использует PyTorch 2.8.0 + CUDA 12.6 с `requirements_pytorch126.txt`. |
...
| `download_lance_3b.bat` | **Интерактивный загрузчик моделей.** Меню для загрузки комбинированных наборов или отдельных компонентов (`Lance_3B_Video`, `Lance_3B`, `Qwen2.5-VL-ViT`, `Wan2.2_VAE`). |
...
| `run_gradio.bat` | **Запуск единого Gradio-демо.** Интерактивный лаунчер с режимами Normal, Custom parameters и `--fp8` для экономии VRAM. |
...
Вы можете запустить локальное Gradio-демо для генерации, редактирования и понимания видео/изображений:

```bash
python lance_gradio.py --server-name 0.0.0.0 --server-port 7860
```

В Windows используйте предоставленный скрипт:
...
| --- | --- | --- |
| `--server-name` | `127.0.0.1` | Адрес привязки сервера. Используйте `0.0.0.0` для внешнего доступа. |
| `--server-port` | `7860` | Порт сервера. |
| `--gpus` | `0` | Список ID GPU через запятую, например `0,1,2,3` для мульти-GPU. |
| `--queue-size` | авто | Максимальный размер очереди Gradio-запросов. |
| `--fp8` | `False` | Конвертировать веса модели в fp8_e4m3fn для снижения потребления VRAM (~50% экономии). |
